...
const CacheSettingsSection: React.FC<CacheSettingsSectionProps> = ({ isOpen }) => {
          ...
          <form onSubmit={editingFolder ? handleEditFolder : handleAddFolder} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-dark-600 text-gray-900 dark:text-white"
            ...
            <div>
              <PathSelector
                value={formData.path}
                onChange={(path) => {
                  // Auto-fill folder name if name is empty
                  if (!formData.name.trim() && path) {
                    const folderName = path.split(/[/\\]/).pop() || path;
                    setFormData({ ...formData, path, name: folderName });
                  } else {
                ...
                type="folder"
                label="Cache Path"
                placeholder="e.g., D:\\Cache\\ImageCache1 (full path required)"
                required
              />
              {formData.path && (
                <div className="mt-2">
        ...
        {stats?.folders && stats.folders.length > 0 ? (
          <div className="space-y-3">
            {stats.folders.map((folder) => (
              <div key={folder.id || folder._id} className="bg-white dark:bg-dark-700 border border-gray-200 dark:border-gray-600 rounded-lg p-4">
...
export default CacheSettingsSection;
